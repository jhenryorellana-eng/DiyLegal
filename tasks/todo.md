...
- [x] **Fase 1 — Slices ligeros Capa B (feeds + estáticos + tools)** ✅ (2026-06-02). 15/15 slices.
- [x] **Fase 2 — AAF Tracker determinista** ✅ (2026-06-02). Motor + PDFs + 2 crons regulatorios.
- [x] **Fase 3 — Capa A núcleo (Supabase Auth + RLS + cifrado)** ✅ (2026-06-02). 8 migraciones aplicadas al proyecto real.
- [ ] Fase 4 — USCIS Torch (caso síncrono) + persistencia de caso  ← **SIGUIENTE**
...
- 2026-06-02 — **FASE 1 COMPLETA (15/15).** Cerrados slices 12-15: Court Intelligence (operational-status `/api/eoir/courts`, court-details `/api/eoir/courts/[slug]`, TRAC judge-stats `/api/eoir/judges[/code]` + `/api/eoir/intelligence/case`) sobre `eoirFetch`+cheerio con persistencia (change-log severidad-alta→CLOSED, cache LRU 500, snapshot de jueces) + 3 crons (eoir-status 6h, court-details diario, trac semanal). Panel `/dev/feeds-test` (slice 15). Códigos reales del juez derivados del enlace TRAC (00347ADL→ADL); `eoirFetch` extendido con decode latin1 para TRAC (ISO-8859-1). 13 crons en vercel.json, 176 tests, 5 gates verdes. **Validado E2E con Playwright**: 11 fuentes ON → 200 con datos reales (EOIR 74 cortes, TRAC 8 jueces ADL, Legal Aid 20 orgs CA), 4 Gemini OFF → 503 esperado. (Commits locales pendientes de push.)
- 2026-06-02 — **FASE 3 COMPLETA (Capa A núcleo).** Proyecto Supabase real "Gyi Legal" (`jxmrhxlzjlxforvhxuuv`), antes vacío. **8 migraciones** versionadas en `supabase/migrations/` y aplicadas vía `apply_migration`: `0001_schema` (15 tablas doc 08 + 8 enums + índices + pgvector), `0002_triggers` (updated_at moddatetime), `0003_rls` (RLS user_id=auth.uid() en 12 tablas usuario), `0004_system_table_policies` (deny-all en idempotency/domain_events/audit_log), `0005_vector_schema` (vector→extensions), `0006_handle_new_user` (auto-provisión de perfil, SECURITY DEFINER search_path=''), `0007_account_purge` (soft-delete + pg_cron purga 30d), `0008_revoke_function_execute` (hardening: revoke EXECUTE de funciones DEFINER). **Cifrado A-Number app-level AES-256-GCM** (`lib/crypto/{aes,anumber}`, clave en .env.local, masking `A1**-***-789`). **Clientes** `@supabase/ssr` (`lib/supabase/{env,client,server,admin,middleware}` + middleware.ts raíz) + tipos generados (`types/supabase.ts`). **Onboarding** (`lib/auth/{consent,profile,account,session}`: consent versionado, idioma, soft-delete). **Aislamiento RLS PROBADO en vivo** (A no ve/edita B) + **E2E real** (`scripts/verify-phase3.ts`: trigger→perfil, A-Number cifrado round-trip, soft-delete, cleanup). **Advisor security 100% limpio.** 285 tests (+3), 5 gates verdes. Blueprint en `tasks/fase-3-blueprint.md`. Pendiente: rotar service_role key (quedó en chat). (Commits locales; push pendiente.)
- 2026-06-02 — **FASE 2 COMPLETA (AAF Tracker determinista).** Motor 100% determinista (sin IA), dinero en centavos, fechas UTC: `branches` (C/D/A/B + getFiscalYear Oct-Dic→+1), `dates` (UTC puro), `constants` (OBBBA/FY/IFR), `regulatory` (snapshot Zod + loadRegulatory cache-first + semilla 10200¢), `fee-amount` (monto+pausas por rama), `estimate-filing-date` (señales EOIR → confianza alta/media/baja), `calculator` (rama+monto+vencimiento+estado+caveats ES/EN). **Regla de vencimiento verificada contra Federal Register / 8 U.S.C. §1808** (anti-alucinación): legacy (presentada <FY2025) → 30-sep anual; post-FY2025 (A/D) → aniversario de presentación (1er a los 365d); rama C misma regla temporal. Endpoints: `POST /api/aaf/calculate` (core), `POST /api/aaf/generate-receipt` (core PDF), `POST /api/aaf/generate-motion` (flag MOTION, PDF, Gemini Pro + **fallback template** → resuelve el bloqueo de billing Pro), `POST /api/aaf/validate` (flag GEMINI), `GET /api/aaf/regulatory/current` (flag REGULATORY_CHECK). PDFs con **pdf-lib** (`lib/pdf/{layout,response,receipt,motion}`). Crons `regulatory-check` (diario 06:00, Gemini Flash grounded — desviación documentada vs doc "Pro") + `icpm-check` (lun 06:00, SHA-256 dedup) → **15 crons** en vercel.json. response-cache TTL 1h. **264 tests** (+88), 5 gates verdes. **Validado E2E con Playwright** (flags ON): calculate→A/overdue/10200¢, legacy→B/2025-09-30, regulatory→10200¢/FY2026, receipt→PDF, motion→PDF fromFallback=true. Blueprint en `tasks/fase-2-blueprint.md`. (Commits locales; push pendiente de autorización.)
