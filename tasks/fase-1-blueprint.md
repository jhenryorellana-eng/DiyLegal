...
- [x] 9. Fee Schedule G-1055 ✅ (lib/feeds/pdf.ts + fee-schedule.ts + endpoint ?form= + cron; 9 tests; pdf-parse v2; feeLines LITERALES sin simplificar; URL verificada; test de contrato anti-alteración de dígitos. Mejora futura: getTable() para bloque exacto por formulario)
- [x] 10. Processing Times ✅ (lib/feeds/processing-times/{index,sync}.ts + endpoint ?form=&office= + cron + scripts/sync-processing-times.ts; 15 tests; node:sqlite dynamic import SOLO en sync.ts; cache-only en el endpoint; mirror jzebedee/uscis verificado E2E real: release 2026-06-01, 43 forms/96 offices/497 times. **Esquema real inspeccionado** (create-*.sql del repo): expone `lowerUnit`+`upperUnit` SEPARADOS (las units difieren/no-numéricas en la fuente, p.ej. "See notes"/null) en vez del `unit` único del doc §7)
- [x] 11. Legal Aid ✅ (lib/legal/legal-aid.ts + endpoint ?state= ; 19 tests; **estrena `eoirFetch`** (lib/eoir/court-intelligence/scrapers/http-client.ts: rate-limit por host + backoff 1/2/4/8s+jitter + detección Cloudflare → EoirCaptchaDetectedError) + cheerio. Selectores VERIFICADOS contra HTML real (.directory-organization-summary, tabla td.label→td). Cache en MEMORIA 1h por estado + fallback stale (sin cron, doc §12). Validado E2E real: 20 orgs CA con nombre/ubicación/tel/web/email)
- [x] 12. Court Intel — Operational Status ✅ (scrapers/operational-status.ts + endpoint /api/eoir/courts?status=&q= + cron eoir-status-sync 6h; persistence/store.ts con change-log severidad alta→CLOSED; 11 tests; verificado E2E real: 74 cortes)
- [x] 13. Court Intel — Court Details ✅ (scrapers/court-details.ts + endpoint /api/eoir/courts/[slug] + cron eoir-court-details-sync diario cap 25; cache LRU 500; 10 tests; E2E real: Adelanto address/phone/6 jueces)
- [x] 14. Court Intel — TRAC Judge Stats ✅ (scrapers/trac-judge-stats.ts + endpoints /api/eoir/judges, /judges/[code], /intelligence/case + cron trac-judge-stats-sync semanal; **código real del juez del enlace (00347ADL→baseCityCode ADL)**; snapshot completo; 11 tests; E2E real: 8 jueces ADL)
- [x] 15. Panel /dev/feeds-test ✅ (page guard NODE_ENV + panel client con 15 sondas; **validado con Playwright**: 11 fuentes ON → 200 datos reales, 4 Gemini OFF → 503 esperado)

**FASE 1 COMPLETA (15/15 slices).** 13 crons en vercel.json. 176 tests verdes. 5 gates verdes. Court Intel usa AAF_CONFIG (flags courtIntelligence/judgeStats/courtChangeAlerts/courtIntelligenceCron).

**Infra creada (acumulada):** `cache.ts`, `gemini/client.ts` (REST+grounding), `pdf.ts` (pdf-parse v2), `eoir/court-intelligence/scrapers/http-client.ts` (`eoirFetch` — compartido con Visa Bulletin y Court Intel) + `cheerio`. 10 crons en vercel.json.
**Notas técnicas Fee Schedule:** pdf-parse v2 API = `new PDFParse({data}).getText().text`; G-1055 = 57 páginas, texto con celdas aplanadas " | "; `getTable()`/`getPageTables()` disponibles para extracción estructurada futura.
**Notas técnicas Processing Times:** mirror `jzebedee/uscis` (GitHub Action publica un SQLite por release diario, tag=fecha, asset `<fecha>.db`). Esquema real (repo `create-*.sql`): `forms(name,description_en,description_es)`, `offices(code,description)`, `processing_time(form_name,office_code,form_subtype,publication_date,range_lower,range_upper,range_*_unit,service_request_date,subtype_info_en/es,...)`. `range_upper`=range[0].value, `range_lower`=range[1].value (USCIS publica [upper,lower]). `range_lower`/`range_upper` pueden ser `null` cuando la unit es textual ("See notes"/"learn more."). `publication_date` constante global por release. node:sqlite SOLO en `sync.ts` (dynamic import); el endpoint lee `data/processing-times-cache.json` (cache-only, sin node:sqlite → portable). Requiere Node≥22.5; fallback documentado: `sql.js` (WASM) o correr el sync en el worker (doc §14).
